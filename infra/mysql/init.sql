...
CREATE TABLE IF NOT EXISTS online_transaction_details (
    transaction_detail_id VARCHAR(50)    NOT NULL,
    transaction_id        VARCHAR(50)    NOT NULL,
    branch_id             VARCHAR(50)    NOT NULL COMMENT 'fulfillment branch (nearest store ships the order)',
    product_id            VARCHAR(50)    NOT NULL,
    is_promo              TINYINT(1)     NOT NULL DEFAULT 0,
    product_uom_code      VARCHAR(20),
    trans_qty             INT            NOT NULL DEFAULT 0,
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Debezium CDC: grant replication privileges to app_user (binlog reader)
-- ─────────────────────────────────────────────────────────────────────────────
GRANT SELECT, RELOAD, SHOW DATABASES, REPLICATION SLAVE, REPLICATION CLIENT ON *.* TO 'app_user'@'%';
FLUSH PRIVILEGES;
