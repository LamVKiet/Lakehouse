...
def seed_transactions(cursor, date_str: str, n_pos=50, n_online=30):
    ...
    for _ in range(n_online):
        ...
        for j in range(n_items):
            ...
            onl_details.append((did, tid, br_id, prd_id, is_promo, unit, qty, amount, variant_size, variant_color, ts))
            total_amount += amount
    ...
    cursor.executemany(
        "INSERT IGNORE INTO online_transaction_details (transaction_detail_id, transaction_id, branch_id, product_id, is_promo, product_uom_code, trans_qty, trans_line_amount, variant_size, variant_color, created_at) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
        onl_details,
    )
