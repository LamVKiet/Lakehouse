...
def load_transaction_details():
    ...
    agg_df = (
        ...
        .agg(f.first("created_at").alias("created_at"),
            f.countDistinct(
                f.when(f.col("is_promo") == 0, f.col("transaction_detail_id"))
            ).cast("bigint").alias("order_total_line"),
            f.sum(f.when(f.col("is_promo") == 0, f.col("trans_qty")).otherwise(0)
            ).cast("bigint").alias("order_total_sku"),
            f.countDistinct(f.when(f.col("is_promo") == 1, f.col("transaction_detail_id"))
            ).cast("bigint").alias("promo_total_line"),
            f.sum(f.when(f.col("is_promo") == 1, f.col("trans_qty")).otherwise(0)
            ).cast("bigint").alias("promo_total_sku"),
            f.max("is_promo").alias("is_promo"),
            f.collect_list(f.array(
    ...
    df = (agg_df.withColumn("report_date", f.date_format(f.col("created_at"), "yyyyMMdd"))
