...
def get_spark_session(app_name: str, extra_jars: str | None = None) -> SparkSession:
    """Create a SparkSession configured for Delta Lake + S3A.
    extra_jars: optional comma-separated local jar paths (e.g. Deequ jar for local
    venv runs). In the Docker image the jar is already on the classpath, so leave
    this None there."""
    builder = (
...
def read_delta_partitions(
    ...
    """
    Read a Delta table with partition pruning via a date-range method (no dict).

    Partition columns auto-resolve by layer:
      bronze -> report_date / report_month ; silver & gold -> ymd / ym.
    Override with day_col / month_col for tables that differ.

    method + values:
      "all"                                    -> no filter
      "from_ym"  / "to_ym"     (scalar)        -> ym  >= / <= value
      "from_ymd" / "to_ymd"    (scalar)        -> ymd >= / <= value
      "between_ym"  / "between_ymd"  ([a, b])  -> col BETWEEN a AND b
      "in_ym"    / "in_ymd"    ([v1, v2, ...]) -> col IN (...)

    Values may carry dashes ("2026-06-01") — stripped to yyyyMMdd / yyyyMM.
    Quoting is type-aware (string/date cols quoted, int cols bare) so Delta prunes.
    Reads via the Delta table (log + Deletion Vectors), never the physical path.

    Examples:
        read_delta_partitions(spark, "bronze", "online_transactions", "in_ymd", [20260601])
        read_delta_partitions(spark, "silver", "transactions", "between_ymd", ["2026-06-01", "2026-06-05"])
        read_delta_partitions(spark, "gold", "daily_customer_sales", "from_ym", "202606")
    """
    from pyspark.sql.types import StringType, DateType
    df = spark.read.format("delta").load(get_s3_path(layer, table))
    day = day_col or ("report_date" if layer == "bronze" else "ymd")
    mon = month_col or ("report_month" if layer == "bronze" else "ym")

    def fmt(c, v):
        v = str(v).replace("-", "")
        return f"'{v}'" if isinstance(df.schema[c].dataType, (StringType, DateType)) else v

    vs = values if isinstance(values, (list, tuple)) else [values]
    if method == "all":            query = "1=1"
    elif method == "from_ym":      query = f"{mon} >= {fmt(mon, vs[0])}"
    elif method == "to_ym":        query = f"{mon} <= {fmt(mon, vs[0])}"
    elif method == "from_ymd":     query = f"{day} >= {fmt(day, vs[0])}"
    elif method == "to_ymd":       query = f"{day} <= {fmt(day, vs[0])}"
    elif method == "between_ym":   query = f"{mon} between {fmt(mon, vs[0])} and {fmt(mon, vs[1])}"
    elif method == "between_ymd":  query = f"{day} between {fmt(day, vs[0])} and {fmt(day, vs[1])}"
    elif method == "in_ym":        query = f"{mon} in ({', '.join(fmt(mon, v) for v in vs)})"
    elif method == "in_ymd":       query = f"{day} in ({', '.join(fmt(day, v) for v in vs)})"
    else: raise ValueError(f"read_delta_partitions: unknown method '{method}'")
...
def enable_auto_optimize(spark: SparkSession, path: str) -> None:
    """Enable Delta Optimized Write + Auto Compact (OSS 3.1+) as table properties so every
    append auto-bin-packs files — replaces manual repartition without throttling parallelism.
    Table must already exist (set after first write, or on startup if it exists)."""
...
def register_glue_table(
    ...
    No column list is stored: with table_type=DELTA, Athena's native Delta reader
    derives the schema (columns, types, partitions) from _delta_log at query time,
    so the catalog stays in sync across schema evolution without re-registration.
    """
    import boto3
    ...
    # Athena uses native S3 client (s3://), not Hadoop S3A (s3a://)
    ...
    table_input = {
        ...
        "StorageDescriptor": {"Columns": [], "Location": glue_location},
        "Parameters": {
            "table_type": "DELTA",
...
def delete_s3_path(path: str) -> None:
    """
    Delete all objects under an S3 path (e.g. s3a://bucket/layer/table/).
    Useful for cleaning up old data in Bronze, Silver, Gold layers.
    """
    import boto3
    ...
    if not bucket_name or not prefix:
        print(f"[s3] Invalid path for deletion: {path}")
        return
    ...
    if deleted_count > 0:
        print(f"[s3] Successfully deleted {deleted_count} objects under {path}")
    else:
        print(f"[s3] No objects found under {path}")

def list_s3_path(path: str) -> list:
    """
    List all object keys under an S3 path.
    Example: list_s3_path("s3a://projectde2026/bronze/")
    """
    import boto3
    from urllib.parse import urlparse
    ...
    if not bucket_name:
        print(f"[s3] Invalid path for listing: {path}")
        return []
    ...
    results = []
    try:
        # Xử lý phân trang nếu thư mục có quá nhiều file
        ...
        print(f"[s3] Error listing path {path}: {e}")
