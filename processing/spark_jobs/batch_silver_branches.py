"""
Spark Batch: bronze.branches -> silver.branches (Daily ETL, SCD1).
Reads T-1 partition. Intra-batch dedup keeps the latest row per branch_id.
Derives _cdc_operation from is_current (1->0/1, 0->2). Drops is_current. 1 row per branch.
...
def transform(bronze_df: DataFrame, run_date: str) -> DataFrame:
    w_dedup = Window.partitionBy("branch_id").orderBy(f.col("updated_at").desc())
    return (
...
def main():
    spark = get_spark_session("Silver-Branches")
    ...
    SILVER_PATH = get_s3_path("silver", "branches")

    bronze_df = read_delta_partitions(spark, "bronze", "branches", "in_ymd", [run_date])
    if bronze_df.count() == 0:
        print(f"[silver.branches] No Bronze rows for {ymd}, skipping.")
        spark.stop()
        return

    silver_df = transform(bronze_df, run_date)
    print(f"[silver.branches] Rows to merge: {silver_df.cache().count()}")

    is_first_run = not DeltaTable.isDeltaTable(spark, SILVER_PATH)
    write_delta_merge(
        spark, silver_df, SILVER_PATH,
        merge_keys=["branch_id"],
        partition_by="region",
    ...
    register_glue_table(spark, "silver", "branches", SILVER_PATH)
    print(f"[silver.branches] MERGE complete for {ymd}.")
    spark.stop()
